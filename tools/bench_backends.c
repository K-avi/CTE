...
int main(){
    ...
    printf("==================================================================================\n");
    printf("      CTE BENCHMARK - ARRAY VS SWAR BITBOARD (-O3 -march=native)                  \n");
    printf("==================================================================================\n\n");
    ...
    // 2. Bitboard SWAR (Vtable / Full Move List)
    clock_t t2 = clock();
    uint64_t moves_bb_cnt = 0;
    for(uint32_t i = 0; i < NUM_SAMPLES; i++){
        moves.size = 0;
        be_bb->gen_all_moves(&moves, table_bbs[i], &hands[i]);
        moves_bb_cnt += moves.size;
        for(uint16_t m = 0; m < moves.size; m++) free_move(&moves.moves[m]);
    }
    clock_t t3 = clock();
    double time_bb = (double)(t3 - t2) / CLOCKS_PER_SEC;

    // 3. Bitboard SWAR Compact 1-Pass (7.1 KB RAM, 100% L1 cache, 0 heap)
    clock_t t4 = clock();
    ...
    clock_t t5 = clock();
    double time_cpt_rnk = (double)(t5 - t4) / CLOCKS_PER_SEC;
    ...
    printf(" | Bitboard SWAR (Vtable Move List)      | %7.3f s | %8.2f Mcoups/s | %7.1f ns  | %12.0f cycles |\n", time_bb, mops_bb, ns_pos_bb, ns_pos_bb * 2.2);
