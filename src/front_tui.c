...
static void tui_menu_quick_match(void) {
  ...
  for (;;) {
    ...
    snprintf(items[5], sizeof(items[5]), "6. Card Render Style : < %s >",
             style_str);
    snprintf(items[6], sizeof(items[6]), "[ START MATCH ]");
    snprintf(items[7], sizeof(items[7]), "[ Back to Main Menu ]");

    for (int i = 0; i < total_items; i++) {
      int row_y = start_y + 3 + (i >= 6 ? i + 1 : i);
      if (i == selected) {
        attron(COLOR_PAIR(PAIR_SELECT) | A_BOLD);
...
static void tui_menu_tournament(void) {
  ...
  for (;;) {
    ...
    snprintf(items[1], sizeof(items[1]),
             "2. Participants      : < %u AI Bots >", (unsigned)nb_p);
    snprintf(items[2], sizeof(items[2]), "3. Match Target      : < %u points >",
             (unsigned)win_score);
    snprintf(items[3], sizeof(items[3]), "[ LAUNCH TOURNAMENT ]");
    snprintf(items[4], sizeof(items[4]), "[ Back to Main Menu ]");

    for (int i = 0; i < total_items; i++) {
      int row_y = start_y + 3 + (i >= 3 ? i + 1 : i);
      if (i == selected) {
        attron(COLOR_PAIR(PAIR_SELECT) | A_BOLD);
        mvprintw(row_y, start_x + 4, " ->  %-48s ", items[i]);
        attroff(COLOR_PAIR(PAIR_SELECT) | A_BOLD);
    ...
    if (ch == KEY_UP || ch == 'k' || ch == 'K') {
      ...
      if (selected == 3) {
        ...
        const char *bot_names[8] = {
            "Cheater_Alpha", "Greedy_Beta", "Random_Gamma", "Dumb_Delta",
            "Cheater_Omega", "Greedy_Zeta", "Random_Eta",   "Dumb_Theta"};
        t_evaluator bot_evals[4] = {eval_cheater, eval_greedy, eval_random,
                                    eval_dumb};

        for (uint8_t i = 0; i < nb_p; i++) {
          snprintf(cfg.participants[i].name, sizeof(cfg.participants[i].name),
                   "%s", bot_names[i]);
          cfg.participants[i].is_human = false;
          cfg.participants[i].evaluator = bot_evals[i % 4];
        }
        ...
        if (init_tournament(&t, &cfg) == e_ok) {
          ...
          erase();
          mvprintw(1, 2, "Running tournament simulations... Please wait...");
          refresh();
          run_tournament(&t);
          ...
          attron(A_BOLD);
          mvprintw(5, res_x + 3,
                   "Rank | %-18s | Won | Lost | Tied | Points | Win%%",
                   "Participant");
          attroff(A_BOLD);
          mvprintw(
              6, res_x + 3,
          ...
          for (uint8_t r = 0; r < t.config.nb_participants; r++) {
            ...
            mvprintw(7 + r, res_x + 3,
                     " %2u  | %-18s | %3u |  %3u |  %3u |  %5u | %4.0f%%",
                     (unsigned)(r + 1), p->name, (unsigned)p->matches_won,
                     (unsigned)p->matches_lost, (unsigned)p->matches_tied,
                     (unsigned)p->total_points, wr);
          }
